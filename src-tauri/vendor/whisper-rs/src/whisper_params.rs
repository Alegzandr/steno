...
impl<'a, 'b> FullParams<'a, 'b> {
    ...
    /// Set `carry_initial_prompt`.
    ///
    /// When enabled, the text given to [`Self::set_initial_prompt`] is
    /// prepended to the prompt of *every* 30-second window rather than only
    /// the first. Without it, an initial prompt carrying domain vocabulary
    /// stops biasing the decoder after the opening thirty seconds, which for
    /// long-form audio is usually not what the caller intended.
    ///
    /// Mirrors `whisper_full_params.carry_initial_prompt`, added upstream in
    /// whisper.cpp 1.7.2.
    ///
    /// Defaults to false.
    ///
    ...
    /// # use whisper_rs::{FullParams, SamplingStrategy};
    /// let mut params = FullParams::new(SamplingStrategy::Greedy { best_of: 5 });
    /// params.set_initial_prompt("Tauri, Ollama, endpoint");
